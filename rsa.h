...
private:
    ...
    void public_string_to_evp(const std::string& public_key_str) {
        this->string_to_evp(&PEM_read_bio_PUBKEY, public_key_str);
    }

    void private_string_to_evp(const std::string& private_key_str) {
...
public:
    rsa(const std::string& public_key = "", const std::string& private_key = "") {
        ...
        if (!public_key.empty() and !private_key.empty()) {
            this->public_string_to_evp(public_key);
            this->private_string_to_evp(private_key);
        }
    }
    ...
    void generate_rsa_keys(const unsigned short size = 2048) {
...
//     Rsa rsa_key(public_key, private_key);
